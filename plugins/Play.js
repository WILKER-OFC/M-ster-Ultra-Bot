...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  const caption =
    `「✦」M-STER ULTRA BOT\n\n` +
    `🔹 Duración: ${duration}\n` +
    `🔹 Vistas: ${(ytInfo?.views || 0).toLocaleString()}\n` +
    `🔹 Autor: ${author}\n` +
    `🔹 Link: ${ytUrl}\n\n` +
    `📋 Formato: MP3\n\n` +
    `📞 Azura ultra 2.0 🔥`
  ...
  // Usar la API directamente con la clave fija
  ...
  console.log(`✅ URL de descarga obtenida: ${directUrl}`)
  ...
  try {
    ...
    await conn.sendMessage(
      chatId,
      {
        audio: audioBuffer,
        mimetype: mime,
        fileName: `${apiTitle.replace(/[^\w\s.-]/g, '_')}.mp3`
      },
      { quoted: m }
    )
    ...
    console.error('Error descargando/enviando audio:', e)
    await conn.sendMessage(
      chatId,
